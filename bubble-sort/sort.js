...
// create the bubble sorting algorith
var bubbleSort = function() {
    // create a boolean to track if there are any changes (if any swaps were done, this will be set to false)
    var sorted = true;

    // a do-while loop will run at least once and then until the final condition is true (which is in the 'while()' at the end of the loop)
    do {
        // set the sorted boolean to true (it will get changed to false if any swaps occur)
        sorted = true;

        // iterate through the entire array of numbers
        for (var i = 0; i < numbers.length - 1; i++) {

            // if a number is greater than the number after it, then we neep to swap it 
            if (numbers[i] > numbers[i+1]) {
                var temp = numbers[i];          // put the first number into a variable
                numbers[i] = numbers[i + 1];    // set the first location to the bigger number
                numbers[i + 1] = temp;          // set the second location to the smaller number
                sorted = false;                 // since we performed a swap, set the boolean to false
            }
        }
    } while (!sorted);  // this will keep running while the 'sorted' boolean is false
}


// just an onclick listener
$(() => {
